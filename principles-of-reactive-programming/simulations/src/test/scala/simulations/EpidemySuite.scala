...
class EpidemySuite extends FunSuite {
  ...
  test("move: should move to a visibly clean room"){
    val es = new EpidemySimulator

    val man = es.persons.head

    val neighborhood = man.neighbors
    .map{ case (row, col) =>
      es.persons
      .filter( p=> p.row == row && p.col == col)
    }

    neighborhood.flatten.foreach(_.sick = true)

    //cannot move
    assert(man.nextRoomOf(man.neighbors) == None, "A man surrouned by visibly Infectious people")

    neighborhood.head.foreach(_.sick = false)

    assert(man.nextRoomOf(man.neighbors) != None, "A man move to clean room")
  }
}
