  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] =
      obs.materialize.flatMap {
        case OnNext(t) => Observable(Success(t))
        case OnError(e) => Observable(Failure(e))
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timeout = System.currentTimeMillis + (totalSec second).toMillis

      obs
      .timestamp
        .takeWhile(_._1 <= timeout)
        .map{case (at, t)=> t}
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      obs
      .flatMap(t=> requestMethod(t).recovered)
