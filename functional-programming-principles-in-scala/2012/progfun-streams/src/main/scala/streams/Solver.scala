...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.foldLeft(Stream[(Block, List[Move])]())((accu, x)=> (x._1, x._2 :: history) #:: accu )
  ...
  lazy val solution: List[Move] = 
  	...
  	else (pathsToGoal.minBy{case (block, moves) => moves.length })._2
}
