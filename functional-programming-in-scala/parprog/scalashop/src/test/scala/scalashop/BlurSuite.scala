...
class BlurSuite extends FunSuite {
  ...
  test("jobs should fragment the divided list") {
    assert(jobs(3, 4) === List((0,1), (1,2), (2, 4)))
    assert(jobs(2, 4) === List((0,2), (2,4)))
    assert(jobs(1, 4) === List((0,4)))
    assert(jobs(32, 32).size === 32)
  }
