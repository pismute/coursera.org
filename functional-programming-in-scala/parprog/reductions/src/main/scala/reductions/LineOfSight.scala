...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    def los(i: Int, acc: Float): Unit = {
      if( i < len ) {
        output(i) = max(input(i) / i, acc)
        los(i+1, output(i))
      }
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def ups(i: Int, acc: Float): Float = {
      if( i < until) {
        ups(i+1, max(acc, input(i)/i))
      } else acc
    }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def dns(i: Int, acc: Float): Unit =
      if(i < until) {
        output(i) = max(acc, input(i)/i)
        dns(i+1, output(i))
      }

    dns(from, startingAngle)
  }
