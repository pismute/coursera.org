...
import FloatOps._
  ...
  test("Leaf add 1body") {
    val b = new Body(123f, 18f, 26f, 0f, 0f)
    val quad = Leaf(17.5f, 27.5f, 5f, Nil).insert(b)
  ...
  test("'insert' should work correctly on a leaf with center (1,1) and size 2") {
    val b1 = new Body(123f, 0.5f, 0.5f, 0f, 0f)
    val b2 = new Body(524.5f, 1.5f, 0.5f, 0f, 0f)

    val l = Leaf(1, 1, 2, Nil)

    val quad = l.insert(b1).insert(b2)

    val expected =
      Fork(Leaf(0.5f,0.5f,1.0f,List(b1)),
        Leaf(1.5f,0.5f,1.0f,List(b2)),
