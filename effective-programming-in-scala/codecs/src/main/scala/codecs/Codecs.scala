...
trait DecoderInstances:
  ...
  /** A decoder for JSON arrays. It decodes each item of the array using the given `decoder`. The resulting decoder
    ...
    *
    * Hint: you should first check that the JSON value to decode is a JSON array, and if it is the case you should
    * iterate over its JSON elements (e.g., with `foldLeft`) to decode them. The value returned by `foldLeft` should be
    * an `Option[List[A]]`. The first parameter of `foldLeft` is the value to return in case the JSON array to decode is
    * empty, so it would be a successful (`Some`) empty list of `A`. Your iteration function should try to decode each
    * element if the previous elements could be successfully decoded, otherwise it should return `None`.
    */
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(xs) =>
        xs.foldRight(Option(List.empty[A])) { (y, ys) =>
          for
            zs <- ys
            z  <- decoder.decode(y)
          yield z +: zs
