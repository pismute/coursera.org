...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    for
      ids   <- client.linksFrom(of)
      names <- WikiResult.traverse(ids.toSeq)(x => client.nameOfArticle(x))
    yield names.toSet
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if (q.isEmpty) WikiResult.successful(None)
        ...
        val (curr, nextQ) = q.dequeue
        curr match {
          case (distance, _) if distance > maxDepth => WikiResult.successful(None)
          case (distance, id) if id == target       => WikiResult.successful(Option(distance))
          case (distance, id)                       =>
            for
              ids    <- client.linksFrom(id).orElse(Set.empty)
              result <- iter(visited + id, nextQ :++ ids.filterNot(visited).map(x => (distance + 1, x)))
            yield result
        }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    WikiResult
      .traverse(titles)(x => client.searchId(x).map(y => x -> y))
      .map { ids =>
        for
          (ta, a) <- ids
          (tb, b) <- ids
      ...
      .flatMap { pairs =>
        WikiResult.traverse(pairs) { (ta, a, tb, b) =>
          breadthFirstSearch(a, b, maxDepth).map(x => (ta, tb, x))
        }
      }
