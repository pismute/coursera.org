...
object WikiResult:
  ...
  /** Asynchronously and non-blockingly transforms a `Seq[A]` into a WikiResult[Seq[B]] using the provided `A =>
    * WikiResult[B]`.
    *
  ...
  def traverse[A, B](ls: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    ls.foldRight(successful(Seq.empty)) { case (a, acc) =>
      f(a).zip(acc).map { case (x, y) => x +: y }
    }
